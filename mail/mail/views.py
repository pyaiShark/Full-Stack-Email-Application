...
def mailbox(request, mailbox):
    ...
    if mailbox == "inbox":
        ...
        emails = Email.objects.filter(
            user=request.user, recipients=request.user, archived=True, trashed=False
        )
    else:
...
def email(request, email_id):
    ...
    if request.method == "GET":
        return JsonResponse(email.serialize()) # serialize the data by calling the function 'serialize'
...
def delete_email(request, email_id):
    ...
    return HttpResponse("Forbidden", 500)
